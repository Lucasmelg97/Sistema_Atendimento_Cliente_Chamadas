...
include 'classe-viewimportvendaservico.php';
include_once "config.php";


class Viewimportvendaservico_DAO{
 ...
 * D = delete 		->		Deleta linhas da tabela
 -----------------------------------------------------------------------------*/
 		...
 		public function retornainfovenda($id){
			...
			$list_query = "SELECT nomeCliente,valorPlano,plano,nomeFunc, nomeLoja, date_format(data,'%d/%m/%Y') as data, acesso, portabilidade, vendedor FROM viewimportvendaservico WHERE idVendaServico=".$id;
            /* Envia a query para o banco de dados e verifica se funcionou */
 			...
 			if( $row = mysqli_fetch_array($result, MYSQLI_ASSOC) ){
 				...
 				$retorno = new Viewimportvendaservico();
                ...
                // $retorno->data = $row["data"];
				 ...
				 $retorno->data= $row["data"];
				 $retorno->acesso= $row["acesso"];
				 $retorno->portabilidade= $row["portabilidade"];
				 $retorno->vendedor= $row["vendedor"];
